        <!-- Start Footer Area  -->
        {{-- <my-footer></my-footer> --}}
        <!-- End Footer Area  -->
        @php
          use App\Models\SiteSetting;
          $data = SiteSetting::first();
        @endphp
        <footer class="edu-footer footer-dark bg-image footer-style-3">
            <div class="footer-top">
              <div class="container">
                <div class="row g-5">
                  <div class="col-lg-3 col-md-6">
                    <div class="edu-footer-widget">
                      <div class="logo">
                        <a href="./">
                          <img class="logo-light" src="{{$data->site_logo}}" alt="Corporate Logo" />
                        </a>
                      </div>
                      <div class="widget-information">
                        <ul class="information-list">
                          <li>
                            <span>Add:</span>{{$data->address}}
                          </li>
                          <li>
                            <span>Call:</span><a href="tel:{{$data->phone}}">+ {{$data->phone}}</a>
                          </li>
                          <li>
                            <span>Email:</span><a href="mailto:{{$data->email}}" target="_blank">{{$data->email}}</a>
                          </li>
                        </ul>
                      </div>
                    </div>
                  </div>
                  <div class="col-lg-3 col-sm-6">
                    <div class="edu-footer-widget explore-widget">
                      <h4 class="widget-title">Quick Link</h4>
                      <div class="inner">
                        <ul class="footer-link link-hover">
                          <li><a href="apa">APA/NOC</a></li>
                          <li><a href="tender">Tender</a></li>
                          <li><a href="career">Career</a></li>
                          <li><a href="form">Forms</a></li>
                        </ul>
                      </div>
                    </div>
                  </div>
                  <div class="col-lg-2 col-sm-6">
                    <div class="edu-footer-widget quick-link-widget">
                      <h4 class="widget-title">Links</h4>
                      <div class="inner">
                        <ul class="footer-link link-hover">
                          <li><a href="frontNotice">Notice</a></li>
                          <li><a href="event-gallary">Events Gallery</a></li>
                          <li><a href="shmu-act">SHMU ACT</a></li>
                          <li><a href="event">News & Event</a></li>
                        </ul>
                      </div>
                    </div>
                  </div>
                  <div class="col-lg-4 col-md-6">
                    <div class="edu-footer-widget">
                      <h4 class="widget-title">Contacts</h4>
                      <div class="inner">
                        <!-- <p class="description">
                          Enter your email address to register to our newsletter
                          subscription
                        </p> -->
        
        
                      <form action="{{ route('store.suscriber') }}" method="POST">
                        @csrf
                        <div class="input-group footer-subscription-form">
                          <input type="email" class="form-control" name="email" required placeholder="Your email" />
                          <button class="edu-btn btn-secondary btn-medium" type="submit">
                            Subscribe <i class="icon-4"></i>
                          </button>
                        </div>
                      </form>
        
                        <ul class="social-share icon-transparent">
                          <li>
                            <a href="{{$data->facebook}}" class="color-fb"><i class="icon-facebook"></i></a>
                          </li>
                          <li>
                            <a href="{{$data->twitter}}" class="color-linkd"><i class="icon-linkedin2"></i></a>
                          </li>
                          <li>
                            <a href="{{$data->linkedIn}}" class="color-ig"><i class="icon-instagram"></i></a>
                          </li>
                          <li>
                            <a href="{{$data->instagram}}" class="color-twitter"><i class="icon-twitter"></i></a>
                          </li>
                          <li>
                            <a href="{{$data->youtube}}" class="color-yt"><i class="icon-youtube"></i></a>
                          </li>
                        </ul>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div class="copyright-area">
              <div class="container">
                <div class="row">
                  <div class="col-lg-12">
                    <div class="inner text-center">
                      <p>
                        &copy; 2023
                        <a href="/" target="_blank">SHMU</a>
                        Designed By
                        <a href="www.ussbd.com" target="_blank">Unicorn</a>. All Rights Reserved
                      </p>
                    </div>
                  </div>
                </div>
              </div>
            </div>
        </footer>



    </div>

    <div class="rn-progress-parent">
        <svg class="rn-back-circle svg-inner" width="100%" height="100%" viewBox="-1 -1 102 102">
            <path d="M50,1 a49,49 0 0,1 0,98 a49,49 0 0,1 0,-98" />
        </svg>
    </div>

    <!-- JS
	============================================ -->
    <!-- Modernizer JS -->
    <script src={{asset("assets/js/vendor/modernizr.min.js")}}></script>
    <!-- Jquery Js -->
    <script src={{asset("assets/js/vendor/jquery.min.js")}}></script>
    <script src={{asset("assets/js/vendor/bootstrap.min.js")}}></script>
    <script src={{asset("assets/js/vendor/sal.min.js")}}></script>
    <script src={{asset("assets/js/vendor/backtotop.min.js")}}></script>
    <script src={{asset("assets/js/vendor/magnifypopup.min.js")}}></script>
    <script src={{asset("assets/js/vendor/jquery.countdown.min.js")}}></script>
    <script src={{asset("assets/js/vendor/odometer.min.js")}}></script>
    <script src={{asset("assets/js/vendor/isotop.min.js")}}></script>
    <script src={{asset("assets/js/vendor/imageloaded.min.js")}}></script>
    <script src={{asset("assets/js/vendor/lightbox.min.js")}}></script>
    <script src={{asset("assets/js/vendor/paralax.min.js")}}></script>
    <script src={{asset("assets/js/vendor/paralax-scroll.min.js")}}></script>
    <script src={{asset("assets/js/vendor/jquery-ui.min.js")}}></script>
    <script src={{asset("assets/js/vendor/swiper-bundle.min.js")}}></script>
    <script src={{asset("assets/js/vendor/svg-inject.min.js")}}></script>
    <script src={{asset("assets/js/vendor/vivus.min.js")}}></script>
    <script src={{asset("assets/js/vendor/tipped.min.js")}}></script>
    <script src={{asset("assets/js/vendor/smooth-scroll.min.js")}}></script>
    <script src={{asset("assets/js/vendor/isInViewport.jquery.min.js")}}></script>
    <script src={{asset("assets/js/vendor/eocjs-newsticker.js")}}></script>
     <!-- img modal -->
     <script src="{{asset('assets/js/custom/img-modal.js')}}"></script>
    <!-- Site Scripts -->
    <script src={{asset("assets/js/app.js")}}></script>
    <script src={{asset("assets/js/custom/index.js")}}></script>
    <script>
        //   plagin js
        $("#example-1").eocjsNewsticker({
          speed: 25,
        });
    </script>
    
</body>

</html>
