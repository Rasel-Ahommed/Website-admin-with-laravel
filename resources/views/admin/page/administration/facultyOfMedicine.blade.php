...
<section class="page-content">
    <div class="page-head">
        <h1>Page / Faculty Of Medicine</h1>
    </div>
    <div class="section shadow">
        ...
        <div class="section-body pt30">
            <form action="{{route('update.facultyofmedicine',['id'=>$data->id])}}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="text-section">
                    <div class="text-content">
                        <div class="text-img" id="hero6">
                            <img class="img-group" src="{{asset('storage/facultysImg/'.$data->img)}}" id="heroimg"/>
                        ...
                        <div class="text-title" id="">
                            <label for="title">
                                Email
                                <input class="input-item" type="text" name="email" id="title" placeholder="Email" value="{{old('email',$data->email)}}" disabled>
                            </label>
                        </div>
                        @error('email')
                            <div class=" text-danger">{{ $message }}</div>
